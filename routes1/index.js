...
exports.cargaComentarios = function(req, res) {
...
console.log("Los comentarios requeridos pertenecen a la noticia con Id = ", req.params.id);

var not_id = req.params.id;

console.log("SELECT * FROM comentarios_noticias WHERE not_id = " + not_id);
conn.query(
  "SELECT * FROM comentarios_noticias WHERE not_id = " + not_id,  
  function(err, results, fields) {
    if (err) {
      conn.end();
      throw err;
	...
	console.log(results.length + " comentarios enviados /n");
